...
namespace streaming_archive {
    static void create_tables (const vector<std::pair<string, string>>& file_field_names_and_types, SQLiteDB& db) {
        ...
        fmt::format_to(statement_buffer_ix, "CREATE INDEX IF NOT EXISTS files_begin_timestamp ON {} ({})", streaming_archive::cMetadataDB::FilesTableName,
        ...
        fmt::format_to(statement_buffer_ix, "CREATE INDEX IF NOT EXISTS files_end_timestamp ON {} ({})", streaming_archive::cMetadataDB::FilesTableName,
        ...
        fmt::format_to(statement_buffer_ix, "CREATE INDEX IF NOT EXISTS files_segment_id ON {} ({})", streaming_archive::cMetadataDB::FilesTableName,
        ...
        fmt::format_to(statement_buffer_ix, "CREATE TABLE IF NOT EXISTS {} ({} TEXT PRIMARY KEY) WITHOUT ROWID",
                       streaming_archive::cMetadataDB::EmptyDirectoriesTableName, streaming_archive::cMetadataDB::EmptyDirectory::Path);
        SPDLOG_DEBUG("{:.{}}", statement_buffer.data(), statement_buffer.size());
        auto create_empty_directories_table = db.prepare_statement(statement_buffer.data(), statement_buffer.size());
                                                               ...
                                                               bool in_specific_segment, segment_id_t segment_id)
    {
        ...
        fmt::format_to(statement_buffer_ix, "SELECT {} FROM {}", get_field_names_sql(field_names), streaming_archive::cMetadataDB::FilesTableName);
        ...
        if (false == file_path.empty()) {
            fmt::format_to(statement_buffer_ix, " {} {} = ?{}", clause_exists ? "AND" : "WHERE", streaming_archive::cMetadataDB::File::Path,
                           enum_to_underlying_type(FilesTableFieldIndexes::Path) + 1);
            clause_exists = true;
        }
        if (in_specific_segment) {
            fmt::format_to(statement_buffer_ix, " {} {} = ?{}", clause_exists ? "AND" : "WHERE", streaming_archive::cMetadataDB::File::SegmentId,
    ...
    static SQLitePreparedStatement get_empty_directories_select_statement (SQLiteDB& db) {
        ...
        fmt::format_to(statement_buffer_ix, "SELECT {} FROM {}", streaming_archive::cMetadataDB::EmptyDirectory::Path,
    ...
    void MetadataDB::init(){
        ...
        file_field_names_and_types[enum_to_underlying_type(FilesTableFieldIndexes::BeginTimestamp)].first =
                streaming_archive::cMetadataDB::File::BeginTimestamp;
        ...
        file_field_names_and_types[enum_to_underlying_type(FilesTableFieldIndexes::NumVariables)].first = streaming_archive::cMetadataDB::File::NumVariables;
        file_field_names_and_types[enum_to_underlying_type(FilesTableFieldIndexes::NumVariables)].second = "INTEGER";

        file_field_names_and_types[enum_to_underlying_type(FilesTableFieldIndexes::IsSplit)].first = streaming_archive::cMetadataDB::File::IsSplit;
        file_field_names_and_types[enum_to_underlying_type(FilesTableFieldIndexes::IsSplit)].second = "INTEGER";

        file_field_names_and_types[enum_to_underlying_type(FilesTableFieldIndexes::SplitIx)].first = streaming_archive::cMetadataDB::File::SplitIx;
        ...
        SPDLOG_DEBUG("{:.{}}", statement_buffer.data(), statement_buffer.size());
        m_upsert_file_statement = make_unique<SQLitePreparedStatement>(m_db.prepare_statement(statement_buffer.data(), statement_buffer.size()));
        statement_buffer.clear();
        ...
        fmt::format_to(statement_buffer_ix, "INSERT INTO {} ({}) VALUES (?) ON CONFLICT DO NOTHING", streaming_archive::cMetadataDB::EmptyDirectoriesTableName,
